%% ===================== Setup =====================
Ndom   = 3;                     % <-- adjust truncation order N
nModes = 2*Ndom + 1;            % k ranges over -N..N
dom    = [0, 5];             % <-- r in [0, Rmax]; r=0 is the left endpoint
idxOf  = @(k) k + Ndom + 1;     % physical index -N..N -> array position 1..2N+1

lambda = 0.5;                   % scalar parameter


%% ===================== Boundary conditions =====================
% All A_k, B_k, C_k = 0 at both ends, except B_0(0) = 1.
...
%% ===================== Assemble chebop =====================
N_op = chebop(@(r, varargin) buildSystem(r, varargin, Ndom, lambda, dom), dom);

N_op.lbc = @(varargin) bcFun(0, varargin, Ndom, 'left');
N_op.rbc = @(varargin) bcFun(dom(2), varargin, Ndom, 'right');
